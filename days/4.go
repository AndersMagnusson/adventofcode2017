...
func FourthFirst() {
	rows := strings.Split(fourthFirstInput, "\n")
	counter := 0
	...
	fmt.Println("Passphrases: ", counter)
}
...
func FourthSecond() {
	...
	fmt.Println("Passphrases: ", len(rows)-counter)
}

func sameWord(w string, wordsSlice []string) bool {
	for _, existingWord := range wordsSlice {
		if len(existingWord) == len(w) {
			same := true
			for _, c := range w {
				if strings.IndexRune(existingWord, c) < 0 {
					same = false
					break
				}
			}
...
var fourthFirstInput = `kvvfl kvvfl olud wjqsqa olud frc
